...
|---------|-------------------|---------|------------|---------------------|-------------|
| **🌐 双向市场** | ✅ 全球首创双向市场<br/>可买卖算力 | ❌ 仅消费 | ❌ 仅消费 | ❌ 仅消费 | ❌ 仅消费 |
| **🔄 双协议原生支持** | ✅ OpenAI + Anthropic API<br/>零适配层损耗 | ⚠️ OpenAI 兼容层<br/>需转换 | ⚠️ OpenAI 兼容<br/>需转换 | ⚠️ 需适配 | ⚠️ 专有 API |
| **🛠️ 双向 API 翻译** | ✅ **完整双向翻译**<br/>• OpenAI ↔ Anthropic<br/>• 工具调用自动适配<br/>• 智能工具过滤<br/>• 循环检测防护 | ⚠️ 单向 OpenAI 兼容<br/>无双向翻译<br/>无循环检测 | ⚠️ 部分工具支持<br/>无双向翻译 | ⚠️ 有限工具支持<br/>无双向翻译 | ✅ 工具支持<br/>但限 Bedrock 模型 |
| **🎯 开发工具集成** | ✅ Codex CLI v0.55.0+<br/>Claude Code v2.0.29<br/>端到端验证 | ⚠️ 需额外配置 | ⚠️ 需额外配置 | ❌ 不支持 | ❌ 不支持 |
| **🔌 部署模式** | ✅ 单二进制/Docker<br/>零依赖部署 | ⚠️ Python 环境<br/>依赖较多 | ☁️ SaaS 托管 | ☁️ 绑定 Cloudflare | ☁️ 绑定 AWS |
| **💾 数据主权** | ✅ 完全本地<br/>SQLite/PostgreSQL | ✅ 本地部署可控 | ❌ 数据过网关 | ⚠️ 边缘节点 | ⚠️ 限 AWS 区域 |
| **📊 监控审计** | ✅ Token 级账本<br/>供应商计费验证 | ✅ 使用量跟踪 | ⚠️ 基础日志 | ⚠️ 基础日志 | ⚠️ CloudWatch |
| **🚀 性能** | ✅ Go 原生<br/>毫秒级延迟 | ⚠️ Python<br/>较高内存 | ☁️ 托管性能 | ✅ 边缘加速 | ✅ 区域化低延迟 |
| **🔓 开源** | ✅ Apache 2.0<br/>完整源码 | ✅ MIT<br/>核心开源 | ❌ 闭源 SaaS | ❌ 闭源 | ❌ 闭源 |
