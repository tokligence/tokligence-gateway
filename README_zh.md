...
Tokligence Gateway 以极低的资源占用提供卓越的性能表现和业界领先的成本效益。

### 与 LiteLLM 的性能对比（v0.3.4，PostgreSQL）

基于 [LiteLLM 官方基准测试](https://docs.litellm.ai/docs/benchmarks)（4 CPU，8GB RAM）：

| 指标 | LiteLLM<br/>（4 实例） | Tokligence v0.3.4<br/>（1 实例） | 性能提升 |
|------|------------------------|--------------------------------|----------|
| **吞吐量** | 1,170 RPS | **11,227 RPS** | **9.6倍** ✨ |
| **P50 延迟** | 100 ms | **49.66 ms** | **2倍** ⚡ |
| **P95 延迟** | 150 ms | **78.63 ms** | **1.9倍** 🚀 |
| **P99 延迟** | 240 ms | **93.81 ms** | **2.6倍** |
| **基础设施** | 4 实例 | **1 实例** | **减少 75%** 💰 |
| **错误率** | 未报告 | **0%** | 完美稳定性 |

**峰值性能**（100 并发）：
- **12,908 RPS** - 绝对最大吞吐量
- **P50: 7.75ms, P95: 16.47ms, P99: 21.15ms** - 亚 100ms 延迟
- **60 秒内处理 774,571 个请求**，零错误

**成本效益**：
- **每美元性能高出 38.4 倍**
- **基础设施成本仅 1/4**（1 实例 vs 4 实例）
- **吞吐量高 9.6 倍**，资源减少 75%

查看 [scripts/benchmark/](scripts/benchmark/) 了解完整测试方法论、详细结果和复现步骤。
...
- 集成指南：
   ...
   - 提示词防火墙：[docs/PROMPT_FIREWALL.md](docs/PROMPT_FIREWALL.md) | [快速开始](examples/firewall/README.md)
