...
|---------|-------------------|---------|------------|---------------------|-------------|
| **🔀 工作模式与路由策略** | ✅ **模型优先的Auto模式**<br/>• **模型优先路由**: 根据模型名路由 (`claude*` → Anthropic, `gpt*` → OpenAI)，再选择协议<br/>• **端点无关**: 同一模型可跨 `/v1/chat` 和 `/anthropic/v1/messages` 使用<br/>• 透传模式（直连）<br/>• 翻译模式（协议转换）<br/>• **成本/延迟权衡**可配置 | ⚠️ **端点优先**<br/>• 仅单向翻译<br/>• OpenAI 格式输入 → 提供商格式输出<br/>• 无反向翻译（无法通过 Anthropic 端点调用 OpenAI）<br/>• 原生格式透传 | ⚠️ **未知**<br/>翻译能力不明<br/>闭源无法确认<br/>可能仅透传 | ✅ **仅透传**<br/>仅边缘代理<br/>无翻译模式<br/>无模型路由 | ✅ **仅透传**<br/>仅 AWS 代理<br/>无翻译模式<br/>AWS 特定路由 |
